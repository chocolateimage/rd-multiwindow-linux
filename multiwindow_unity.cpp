...
void CustomWindow::updateThings() {
    ...
    if (waylandType == WaylandType::KDE) { // Wayland doesn't support actual window movement. We have to "smuggle" data
        ...
        if (!hyprReady) {
            qInfo() << "[window" << customId << "] initializing on Hyprland, addr=" << hyprAddress.c_str()
                    << "move to" << finalX << finalY
                    << "size" << finalWidth << "x" << finalHeight << "opacity" << finalOpacity
...
Hyprctl::Hyprctl() {
    ...
    const char* debugEnv = getenv("RD_MULTIWINDOW_DEBUG");
    debug = (debugEnv != nullptr && strcmp(debugEnv, "1") == 0);
    ...
    if (!versionStr.empty()) {
        ...
        if (sscanf(versionStr.c_str(), "Hyprland %d.%d", &major, &minor) >= 2) {
            ...
            qInfo() << "Hyprland version" << major << "." << minor
                    << "- using" << (useNewSyntax ? "new Lua" : "legacy hyprlang") << "syntax";
        } else {
...
#endif
    qInfo() << "Hyprctl socket:" << socketPath.c_str() << "syntax:"
            << (useNewSyntax ? "Lua" : "hyprlang") << "debug:" << (debug ? "on" : "off");
}

void Hyprctl::sendMessage(std::string message) {
    if (debug) {
        qDebug() << "[hyprctl] >>> (async)" << message.c_str();
    }
...
bool Hyprctl::sendMessageSync(std::string message) {
    if (debug) {
        qDebug() << "[hyprctl] >>> (sync)" << message.c_str();
    }
    ...
    if (debug) {
        qDebug() << "[hyprctl] <<<" << reply.c_str();
    }
    if (reply != "ok") {
        if (reply.rfind("warning:", 0) == 0) {
            // Hyprland warnings (e.g. "Window is fullscreen") are not fatal.
...
void Hyprctl::moveWindow(std::string window, int x, int y) {
    if (useNewSyntax) {
        sendMessage("dispatch hl.dsp.window.move({ x = " + std::to_string(x) + ", y = " + std::to_string(y) +
                    ", window = \"" + window + "\" })");
    } else {
        sendMessage("dispatch movewindowpixel exact " + std::to_string(x) + " " + std::to_string(y) + "," + window);
    }
}
...
void Hyprctl::floatWindow(std::string window) {
    if (useNewSyntax) {
        ...
        sendMessageSync("dispatch setfloating " + window);
    }
}
...
void setMainWindowGeometry(int x, int y, int w, int h) {
    ...
    if (waylandType == WaylandType::Hyprland) {
                    ...
                    while (pos != std::string::npos) {
                        ...
                        if (region.find("RD_CHILD") == std::string::npos) {
                            ...
                            if (addrPos != std::string::npos) {
                                ...
                                qInfo() << "[main window] resolved ->" << mainWindowHyprAddress.c_str();
                                return;
                            }
                        }
        ...
        static bool mainFloated = false;
        if (!mainFloated) {
            mainFloated = true;
            qInfo() << "[main window] decoupling fullscreen state and floating";
            if (hyprctl->useNewSyntax) {
                hyprctl->sendMessageSync(
                    "dispatch hl.dsp.window.fullscreen_state({ internal = 0, client = 2, "
                    "window = \"" + mainWindowHyprAddress + "\" })");
            } else {
                hyprctl->sendMessageSync("dispatch fullscreen 0");
            }
            hyprctl->floatWindow(mainWindowHyprAddress);
            hyprctl->setProp(mainWindowHyprAddress, "no_blur", "true");
            hyprctl->setProp(mainWindowHyprAddress, "no_anim", "true");
        }
        if (invisible) {
            qInfo() << "[main window] hiding (opacity=0.01)";
            hyprctl->setProp(mainWindowHyprAddress, "opacity", "0.01");
        } else {
            // Always center main window when showing (fullscreen window = 0,0)
            qInfo() << "[main window] showing centered at 0,0";
            hyprctl->setProp(mainWindowHyprAddress, "opacity", "1");
            hyprctl->moveWindow(mainWindowHyprAddress, 0, 0);
        }
    } else {
        if (main_window_handle == 0) {
            return;
                ...
                std::thread([customWindow, needle, cid]() {
                    ...
                    for (int i = 0; i < 100; i++) {
                        ...
                        if (pos != std::string::npos) {
                            ...
                            qInfo() << "[window" << cid << "] resolved address:" << addr.c_str();
                            break;
                        }
                    }
                    if (addr.empty()) {
...
extern "C" WINAPI const char* show_window(HWND window) {
    ...
    CustomWindow* customWindow = (CustomWindow*)window;
    qInfo() << "[window" << customWindow->customId << "] show_window()";
    QMetaObject::invokeMethod(
        app,
        [customWindow]() {
...
extern "C" WINAPI const char* hide_window(HWND window) {
    ...
    CustomWindow* customWindow = (CustomWindow*)window;
    qInfo() << "[window" << customWindow->customId << "] hide_window()";
    QMetaObject::invokeMethod(
        app,
        [customWindow]() {
